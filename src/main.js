...
async function main() {
	...
	*/
	...
	function update_cam_transform(frame_info) {
		...
		*/
		const sim_time = frame_info.time;

		var r = cam_distance_base * cam_distance_factor;

		const mat_rotation_z = mat4.create();
		const mat_rotation_y = mat4.create();

		mat4.fromZRotation(mat_rotation_z, cam_angle_z);
		mat4.fromYRotation(mat_rotation_y, cam_angle_y);

		// Example camera matrix, looking along forward-X, edit this
		let rotation = mat4_matmul_many(mat4.create(), mat_rotation_y, mat_rotation_z);
		...
		mat4_matmul_many(frame_info.mat_turntable, look_at, rotation) // edit this
	}
